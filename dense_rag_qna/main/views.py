...
def document_upload(request):
    if request.method == 'POST':
        ...
        doc_name = document.name.replace('.pdf', '')
...
def retrieve_information(request, doc_name):
    if request.method == 'POST':
        ...
        similar_docs = retrieve_docs(doc_name.replace('.pdf', ''), o_query, CACHE_PATH, PERSISTENT_DIR)
        ...
        return JsonResponse({"response": response.content, 'o_query': o_query}, status=205)
    else:
        return JsonResponse({"error": "Invalid HTTP method."}, status=405)